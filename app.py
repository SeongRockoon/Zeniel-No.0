...
.app{display:grid;grid-template-columns:clamp(210px,15vw,260px) minmax(0,1fr);gap:10px;padding:8px;width:100vw;height:calc(100vh - 58px);overflow:hidden}
...
.stage{position:relative;flex:0 1 auto;width:100%;max-width:calc((100vh - 106px) * 1.7779);max-height:calc(100vh - 106px);aspect-ratio:1672/941;margin:auto;background:#ddd;border-radius:8px;overflow:hidden;border:1px solid #b9c3c0}.stage>img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;pointer-events:none}
...
@media(max-width:900px){.app{grid-template-columns:190px minmax(0,1fr);gap:6px;padding:5px;height:calc(100vh - 52px)}.panel{padding:7px;gap:5px}.sub{display:none}.dropbox{min-height:42px}.person{font-size:10px;padding:3px 4px}.btn{padding:6px;font-size:11px}.stage{max-width:calc((100vh - 90px) * 1.7779);max-height:calc(100vh - 90px)}}
</style></head><body>
<div class="app">
 <aside class="panel">
