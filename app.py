...
<div class="app">
 ...
 <main class="stage-wrap"><div class="stage-head"><b>업무 상황판 <small style="color:#087f5b">v5.1</small></b><span id="status" class="status">변경사항 없음</span></div><div id="stage" class="stage"><img src="data:image/jpeg;base64,{{IMAGE}}"></div></main>
...
const stage=document.getElementById('stage'), dateEl=document.getElementById('workDate'); let people=[], dirty=false, drag=null, pendingPlacement=null;
...
function endDrag(ev){if(!drag)return;let p=drag.p,originPlace=p.place,sr=stage.getBoundingClientRect(),placed=false;if(ev.clientX>=sr.left&&ev.clientX<=sr.right&&ev.clientY>=sr.top&&ev.clientY<=sr.bottom){let x=Math.max(1,Math.min(99,(ev.clientX-sr.left)/sr.width*100)),y=Math.max(1,Math.min(99,(ev.clientY-sr.top)/sr.height*100));let z=ZONES.find(z=>x>=z.x&&x<=z.x+z.w&&y>=z.y&&y<=z.y+z.h),zone=z?.id||'기타';if(originPlace!=='stage'){pendingPlacement={p,x,y,zone};placed='pending'}else{p.x=x;p.y=y;p.zone=zone;placed=true}}
...
document.querySelectorAll('.shift-btn').forEach(btn=>btn.onclick=()=>{if(!pendingPlacement)return;let {p,x,y,zone}=pendingPlacement;p.x=x;p.y=y;p.zone=zone;p.place='stage';p.shift=btn.dataset.shift;pendingPlacement=null;closeShiftModal();changed();render()})
